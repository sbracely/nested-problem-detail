...
@Service
@Validated
public class Boot3MvcProblemDetailService {

    private static final Logger logger = LoggerFactory.getLogger(Boot3MvcProblemDetailService.class);

    @Boot3MvcCheckName(nameIndex = 0, problemDetailRequestIndex = 1)
    public void createProblemDetail(@NotBlank(message = "name must not be blank")
                                    @NotNull(message = "name must not be null") String name,
                                    @Valid Boot3MvcProblemDetailRequest problemDetailRequest) {
