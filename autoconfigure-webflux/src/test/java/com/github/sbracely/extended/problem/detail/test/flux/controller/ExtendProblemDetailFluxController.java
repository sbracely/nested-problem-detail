...
import com.github.sbracely.extended.problem.detail.test.flux.reuqest.valid.annocation.CheckMultipartFile;
...
//    }
    ...
    public void matrixVariable(@PathVariable String id, @MatrixVariable @Size(max = 2, message = "最大长度是 2") List<String> list) {
        log.info("id: {}, list: {}", id, list);
    }
    ...
    public void requestParam(@NotBlank(message = "参数不能为空") String param, @NotNull(message = "参数2不能为空") @NotNull(message = "参数2不能为null") String param2) {
        log.info("param: {}, param2: {}", param, param2);
    }

    @GetMapping("/request-part")
    public void requestPart(@RequestPart(required = false) @CheckMultipartFile(extensionIncludeMessage = "文件类型不支持", extensionInclude = "txt", requiredMessage = "文件不能为空") MultipartFile file) {
        log.info("file: {}", file);
    }

    @GetMapping("/request-other")
    public void requestOther(@SessionAttribute(required = false) @NotBlank(message = "sessionAttribute 不能为空") String sessionAttribute, @RequestAttribute(required = false) @NotBlank(message = "requestAttribute 不能为空") String requestAttribute, @Value("") @NotBlank(message = "value 不能为空") String value) {
...
//    }
    ...
    public void unsatisfiedRequestParam() {
        ...
        throw new UnsatisfiedRequestParameterException(List.of("type=1", "exist"), org.springframework.util.CollectionUtils.toMultiValueMap(Map.of()));
    }
